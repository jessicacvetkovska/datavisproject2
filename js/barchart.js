class BarChart {
  ...
  constructor(_config, _data) {
    ...
    this.config = {
      ...
      xAxisLabel: _config.xAxisLabel || 'priority'
    }
    this.data = _data;
    this.initVis();
  }
    ...
    bars
        .on('mouseover', (event,d) => {
          d3.select('#tooltip')
            ...
            .html(`<div class="tooltip-label"></div><div class="tooltip-label">${vis.config.yAxisLabel}: ${d3.format('.1f')(vis.yValue(d))}</div>`)
        })
        .on('mousemove', (event) => {
          d3.select('#tooltip')
